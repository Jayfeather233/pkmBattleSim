...
public:
    ...
    base6 species_points; // 种族值是反映不同种类宝可梦之间各项能力大致情况的数值
    ...
    std::vector<std::pair<int, float>> poss_items;     // 可能的物品, id - possibility
    ...
    pkm_base(const std::string &nm, base6 sp, std::array<element_types, 2> et, int idx, const std::string &cat,
...
public:
    ...
    std::string get_name() const { return name.length() == 0 ? base_name : name; }
    gender gend;
    u_char level;
    size_t exp_curr;  // current level exp
    size_t exp_total; // total exp gained
    base6 IV;         // individual value, random generate at catch
              // 个体值是参与决定每只宝可梦能力的一组数值。个体值与生俱来，无法改变，是每只宝可梦的一个隐藏参数。
    base6 base_points; // aquired throuth battle
    ...
    pkm(const pkm_base &pb, const std::string &namex, const std::string &get_t, const std::string &get_p, gender gen,
