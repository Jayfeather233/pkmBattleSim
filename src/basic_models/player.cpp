...
{
    Json::Value J =
        string_to_json(readfile(filepath, "{\"badge\":0,\"name\": \"\", "
                                          "\"party_pkm\":[],\"chest_pkm\":[],\"pls\":0,\"las_pls\":0,\"money\":0, "
    ...
    las_store_place = &placs[J["las_pls"].asInt()];
    money = J["money"].asInt();
    st = settings(Ja2set(J["settings"]["type"]), Ja2set(J["settings"]["enable"]));
    sig_save = false;
}
...
settings::settings() {}
settings::settings(const std::set<std::string> &ut, const std::set<std::string> &ue) : user_types(ut), user_enables(ue)
{
}
...
pkm *player::get_choose_pkm()
{
    if (mt.menu_choose_pokemon > 6) {
        return static_cast<int>(chest_pkm.size()) > mt.menu_choose_pokemon - 6 ? &chest_pkm[mt.menu_choose_pokemon - 6]
                                                                               : &chest_pkm[0];
    }
    else {
...
void player::gain_exp(size_t position, size_t exp)
{
    ...
    if (p.level != oo) {
        ...
        this->output2user(fmt::format("{}\n"
                                      ...
                                      "{}: {} -> {}\n"
                                      "{}: {} -> {}",
                                      get_levelup_text(p), base6str[0], pst.hp, p.stat.hp, base6str[1], pst.atk,
                                      p.stat.atk, base6str[2], pst.df, p.stat.df, base6str[3], pst.stk, p.stat.stk,
                                      base6str[4], pst.sdf, p.stat.sdf, base6str[5], pst.spd, p.stat.spd));
    }
}
