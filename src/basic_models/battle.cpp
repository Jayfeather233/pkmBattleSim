...
std::string battle_main::to_string(int side)
{
    return fmt::format("对方: {}\n己方: {}", half_to_string(side ^ 1), half_to_string(side));
}

void battle_main::sort_moves(sort_method method)
{
...
std::vector<std::pair<int, int>> battle_main::aff_pkms(const move_struct &ms)
{
...
void battle_main::set_prior_fix()
{
 ...
 */
void battle_main::get_gained_exp(int side, int pos)
{
    refresh_seened_pkm();
    if (p[side ^ 1]->badge < 0)
        return;
    ...
    for (auto u : to_gain) {
        ...
        if (i != p[side ^ 1]->party_pkm.size()) {
            p[side ^ 1]->gain_exp(i, (a * b * l / (5.0 * s) * std::pow((2.0 * l + 10) / (l + lp + 10.0), 2.5) + 1) * e *
                                         f);
        }
    }
}
...
void battle_main::execute_move(const move_struct &ms)
{
    // TODO: text here
...
void battle_main::field_effect(field_status fs, pkm *p)
{
    // TODO text_here
...
void battle_main::execute_change_pkm(const pkm_list_item &ms)
{
    // TODO: text here
...
void battle_main::exchange_pkm(bool first_turn)
{
    ...
    for (pkm_list_item &u : pls) {
        for (auto ms : moves) {
            if (ms.use_item == -1 && ms.exchange_pkm == -1 && skill_list[ms.from->skills[ms.skill_id]]->uid == 228 &&
...
void battle_main::end_turn_proc()
{
...
int battle_main::process_turn(bool first_turn)
{
    if (escape_detec()) {
        return 2;
    }
