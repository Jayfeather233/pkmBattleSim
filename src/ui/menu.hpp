...
public:
    ...
    std::map<std::string, std::vector<text_menu *>> app_options; // appended options for user_type, like Pokémon Refresh
                                                                 // in Sun/Moon, this can allow users to choose what
                                                                 // features to enable. Since it only involves simple
                                                                 // status change, this can be read from file
    ...
    std::function<std::vector<std::string>(const player &p)> get_choose_set; // call to get item list
    ...
    text_menu(std::string titlex, std::string choose_textx, std::vector<text_menu *> optionsx,
              std::function<void(player &p)> actionx = nullptr, bool is_choosex = false,
              std::function<void(player &p, int id)> choose_cbx = nullptr,
              std::function<std::vector<std::string>(const player &p)> get_choose_setx = nullptr, bool need_back = true,
              std::string uidx = "");
...
void run_text_menu(player &p, const text_menu *px, std::function<void()> save, const text_menu *stop = nullptr);

extern std::map<std::string, std::function<void(player &, int)>> choose_callback_mapper;

extern std::map<std::string, std::function<void(player &)>> action_mapper;

extern std::map<std::string, std::function<std::vector<std::string>(const player &)>> get_choose_set_mapper;